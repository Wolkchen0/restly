        ...
        import("@/utils/pdf-export").then(({ exportToPDF }) => {
            exportToPDF({
                ...
                rows: [
                    ...
                    ["Cost of Goods Sold", `-$${stats.cogs.toLocaleString()}`, `${stats.cogsPct}%`, "Goal: <30%"],
                    ["Labor Cost", `-$${stats.labor.toLocaleString()}`, `${stats.laborPct}%`, "Warning: Near 35%"],
                    ["Operating Expenses", `-$${stats.operatingEx.toLocaleString()}`, `${stats.operatingExPct}%`, "Rent, Utilities"],
                    ["", "", "", ""],
                    ["NET INCOME", `$${netProfit.toLocaleString()}`, `${profitMargin}%`, ""],
                    ["", "", "", ""],
                    ["Weekly Breakdown", "", "", ""],
                    ...MONTHLY_DATA.map(d => [d.name, `$${d.revenue.toLocaleString()}`, `$${d.cogs.toLocaleString()} COGS`, `$${d.profit.toLocaleString()} Profit`]),
                ],
                sectionRows: [5, 7],
